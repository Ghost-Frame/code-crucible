...
fn client() -> Result<SkillBackend, ToolError> {
    SkillBackend::new().map_err(|e| ToolError::IoError(e.to_string()))
}

fn backend_err(e: crate::skill_backend::SkillBackendError) -> ToolError {
...
pub fn skill_lineage(input: SkillLineageInput) -> ToolResult {
    let skill_id = input
        ...
        .ok_or_else(|| ToolError::MissingField("skill_id".into()))?;
    let client = client()?;
    let result = client.get_lineage(skill_id).map_err(backend_err)?;
